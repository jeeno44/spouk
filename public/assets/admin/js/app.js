...
$(document).ready(function () {
    ...
    $('#master-check').change(function(){
        if ($(this).prop('checked') == true) {
            ...
            if ($('.select-items:checked').length > 0)
                $('.destroy-btn').css('display', 'inline-block');
        } else {
            $('.select-items').prop('checked', false);
            $('.destroy-btn').css('display', 'none');
        }
    });
    if ($('input[name=slug]')) {
        $('input[name=name]').bind('keyup change paste onpaste', function () {
    ...
    $(document).on('change', '.switcher', function(e) {
        ...
        var activity = 0;
        if ($(this).is(':checked')) activity = 1;
        var data =  {_token:  $('input[name=_token]').val(), id: id, model: model, activity: activity};
        $.ajax({
            url: '/api/switcher',
            type: 'POST',
            data: data
    ...
    $('#upload-img').change(UploadSingleImage);
    $('#upload-images').change(UploadImages);
...
function sort(elem, url) {
    $(elem).sortable({
        placeholder: "ui-state-highlight col-sm-12",
        axis: "y",
        handle: '.fa-arrows',
        update: function(e, ui) {
            var positions = $(this).sortable('toArray');
            var data = {_token:  $('input[name=_token]').val(), positions: positions};
            $.ajax({
                url: url,
                type: 'POST',
                data: data,
...
function UploadSingleImage ()
{
    ...
    var error = '';
    jQuery.each($('#upload-img')[0].files, function(i, file) {
    ...
    if (error != '') {
        $('#info').addClass('alert alert-danger').html('Неверный формат изображения').show();
    } else {
        $.ajax({
            url: '/api/upload',
            type: 'POST',
            ...
            processData:false,
            beforeSend: function() {
                $('#upload-result').attr('src', '/assets/admin/img/preloader.gif');
                $('#info').hide();
            },
            success: function(data) {
                $('#img').val(data);
                $('#upload-result').attr('src', '/uploads/images/' + data);
                $('#info').hide();
            },
...
function UploadImages ()
{
    ...
    data.append('_token', $('input[name=_token]').val());
    if (error != '') {
        $('#info-block').addClass('alert alert-danger').html('Неверный формат изображения').show();
    } else {
        $.ajax({
            ...
            processData:false,
            beforeSend: function() {
                $('#plus-image').attr('src', '/assets/admin/img/preloader.gif');
            },
            success: function(data) {
                $('#upload-images').before(data);
                $('#plus-image').attr('src', '/assets/admin/img/plus.png');
            },
            error: function(data) {
                if (XMLHttpRequest.statusText != 'abort') {
                    alert(XMLHttpRequest.statusText);
...
function convertToSlug(text)
{
    ...
    text = text.replace(/[А-яёЁ]/g,replacer);
